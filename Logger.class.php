...
class Logger
	{
		...
		protected $Priorities = array("Critical", "High", "Medium", "Low", "Trivial");
		protected $Priority = 'Medium';
		protected $Types = array("Error", "Warning", "Notice", "Info", "Debugging");
		...
		public function SetFileName($FileName, $IncludeDate = TRUE)
			{
				$this->FileName = ($IncludeDate == TRUE) ? $FileName.'-'.date("Y-m-d").'.log' : $FileName.'.log';
			} # end SetFileName()
		...
		public function OpenLogFile()
			{
				...
				if (!is_resource($this->FileHandle))
					{
						$Exists = (file_exists($this->FilePath.$this->FileName)) ? TRUE : FALSE;
				...
				if (is_resource($this->FileHandle))
					{
						if ($LogDetails == TRUE)
							{
								...
								if ($Check == FALSE) { throw new Exception("Unable to write log details."); }
							}
						...
						if ($Check == FALSE) { throw new Exception("Unable to write log message."); }
